...
import testcases.MainTestNG;

/**
 * The SwingTest class is used to provide UI capabilities. UI for
...
public class SwingUI extends JPanel {
	...
	transient MainTestNG test = new MainTestNG();

	String configFile = "config.properties";
	String browse = "Browse";
	public static final int MODE_OPEN = 1;
	public static final int MODE_SAVE = 2;
	JButton ltestcasebrowse, ltestsuitebrowse, belementLoc, bbrowserDriver, bClear, bClose, bscreenshot, bGenerate;
	JTextField ftestcase, ftestsuite, felementLoc, fbrowserDriver, fscreenshot, fURL;
	...
	public SwingUI() {
		...
		grid.gridy = 0;
		grid.insets = new Insets(5, 20, 5, 5);
		ftestcase.setToolTipText("Path of excel file in which testcases are written and file type should be .xlsx");
		...
		ftestsuite.setToolTipText("Path of excel file- TestSuite and file type should be .xlsx");
		grid.gridx = 1;
		grid.gridy = 1;
		grid.insets = new Insets(5, 20, 5, 5);
		...
		grid.gridx = 1;
		grid.gridy = 2;
		felementLoc.setToolTipText("Path of elemet locators properties file and file type should be .properties");
		...
		fbrowserName = new JComboBox<>(brNamesList);
		...
		grid.gridx = 1;
		grid.gridy = 4;
		fbrowserDriver.setToolTipText(
				"exe of specific browser server (IEDriverServer.exe, ChormeDriverServer.exe) and file type should be .exe");
		...
		fscreenshot.setToolTipText("Screenshots on failure of testcase to be saved");
		grid.gridx = 1;
		grid.gridy = 5;
	...
	private void addbutton(JButton ltestcasebrowse2, final JTextField textField) {
		ltestcasebrowse2.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				buttonActionPerformed(textField);
	...
	private void buttonActionPerformed(JTextField textField) {
	...
	private void generatePropFile() {
		...
		if (!ftestcase.getText().endsWith(".xlsx") || ftestcase.getText().length() == 0) {
			status = false;

			JOptionPane.showMessageDialog(ftestcase, "Field is blank or invalid file type");

		} else if (!ftestsuite.getText().endsWith(".xlsx") || ftestcase.getText().length() == 0) {
			status = false;

			JOptionPane.showMessageDialog(ftestsuite,
					"testsuite: invalid file type.Please add file with  extension .xlsx");
		} else if (!felementLoc.getText().endsWith(".properties")) {
			status = false;

			JOptionPane.showMessageDialog(felementLoc,
					"Element Locators: invalid file type.Please add file with  extension .properties");
		} else if (!fbrowserDriver.getText().endsWith(".exe")) {
			status = false;

			JOptionPane.showMessageDialog(fbrowserDriver,
		...
		if (ftestcase.getText().length() == 0) {
			...
			try (FileInputStream in = new FileInputStream(configFile);) {
				...
				if (props.getProperty(testsuite) != null && ftestsuite.getText().length() == 0) {
					ftestsuite.setText(props.getProperty(testsuite));
				}
				if (props.getProperty(locators) != null && felementLoc.getText().length() == 0) {
					felementLoc.setText(props.getProperty(locators));
				}

				if (props.getProperty(browserdriver) != null && fbrowserDriver.getText().length() == 0) {
			...
			try (FileInputStream in = new FileInputStream(configFile);
				...
				if (fbrowserName.getSelectedItem().toString() != null) {
					props.setProperty(browsername, fbrowserName.getSelectedItem().toString());
				}
